...
### Runtime – Agent loop 
Before being fed to the agent, the input Pennylane program is parsed into its Abstract Syntax Tree (AST) to provide structural context to the underlying LLM. 

Each node in the AST will be query against the graph database to fetch parts of the knowledge graph that is relevant to the conversion of the input program. 

All the provided context will be structured together with a system prompt and fed to an LLM, that will return an output Qiskit program. Both the input Pennylane and output Qiskit programs will be executed, and their responses will be either fed back to an LLM or a human for interpretation of equivalence. If equivalence fails, the agent iterates and retries previous steps for a few times. 
...
|-------|----------------|----------------|-----|----------|
| Stage 1 | Manually constructed, small | No | Local model (Ollama 3) | Programs don't always run, but at least look in shape. |
| Stage 2 | LLM generation, medium | Maybe? | Commercial API (Claude, Gemini, etc) | TBD |
| Stage 3 | Parsed from docs and LLM generation | Yes | Commercial API (Claude, Gemini, etc) | TBD |


## Challenges, learnings and Insights
The fundamental challenge for this project is establishing equivalence between a Pennylange and a Qiskit program, which are inherently different programming paradigms and designed for different purposes. To validate equivalence, whether a human or machine judge, must understand what the programs do and what their outputs mean. This means knowledge of quantum computing and programming are pre-requisite. There are also limited non-structured examples on how to convert between Pennylane and Qiskit, meaning there's little available data to construct relation edges.

Knowledge about Pennylane, Qiskit and quantum programming that allows one to read program *between the lines* is crucial for this project, but also takes time and practice to acquire. 

## Next Steps
### Use foundational models as opposed to open-source models 
Latest foundational models such as GPT-5, Claude Sonnet 4.5 and Gemini 2.5 are not limited to the computational resources of a personal computer, and can offer the best ability for coding tasks to date. Consuming APIs of these foundational models for our agent likely means more performant coding assistants.
### Scale KG construction
Pennylange and Qiskit documentations are both built using Sphinx. Using the structure of Sphinx documentation, we can parse out interface descriptions to serve as informational nodes in KGs. Similarly, we find Pennylane and Qiskit programs that are of similar functions or existing examples of program conversion between the two as source data. Using these source data, either manually or through LLM generation, we add relation edges to KGs. 
### Index KGs with database for retrieval
LLMs have a limited context window, and it’s best to only feed the most relevant context to them such that they don't side track or hallucinate.  For this reason, we store and index the KG into a database for efficient retrieval. With that, only the informational nodes and relation edges relevant to the input program will be queried and provided as context to the LLMs in the agent loop.
